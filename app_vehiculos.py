...
st.title("⚡ Catálogo y Ventas de Vehículos Electrificados en Chile")
...
with tab_ventas:
    ...
    if not df_ventas.empty:
        ...
        def crear_grafico_ventas(tipo, color, titulo):
            ...
            if len(ventas) == 0:
                fig = px.bar(title=f"{titulo} (Sin ventas)"); fig.update_layout(xaxis_visible=False, yaxis_visible=False); return fig
    ...
    if not df_homologados.empty:
        ...
        with col_t2:
            ...
            if 'Capacidad kWh' in df_tech_filtro.columns:
                ...
                if len(df_kwh_filtro) > 0:
                    ...
                    st.info("No hay registros de capacidad.")

        st.divider()
        st.subheader(f"Evolución de Capacidad de Almacenamiento Inyectada a Chile ({tipo_elegido})")
        st.markdown(f"*(MWh estimados por año cruzando el volumen de ventas con la batería promedio de cada marca para **{tipo_elegido}**)*")
        ...
        if not df_ventas.empty and 'Capacidad kWh' in df_homologados.columns:
            ...
            if not df_energia_tech.empty and df_energia_tech['MWh Añadidos'].sum() > 0:
                ...
                st.info(f"No hay suficientes datos de ventas o capacidad declarada para calcular la energía de los modelos {tipo_elegido}.")

        st.divider()
        st.subheader("Evolución General de Energía Inyectada a la Red (Todas las Tecnologías)")
        st.markdown("*(MWh totales estimados por año, sumando el aporte simultáneo de EV, PHEV y HEV)*")
        ...
        if not df_ventas.empty and 'Capacidad kWh' in df_homologados.columns:
            ...
            if not df_energia_global.empty:
                ...
                fig_energia_global = px.bar(
                    df_energia_global, x='Año', y='MWh Añadidos', color='Tipo',
                    color_discrete_map=colores_tech, text_auto='.1f'
                )
...
# --- PESTAÑA 3: CATÁLOGO (NUEVO SISTEMA DE FILTROS AVANZADOS) ---
with tab_catalogo:
    ...
    if not df_homologados.empty:
        ...
        # 1. Búsqueda por texto (Para búsquedas rápidas)
        ...
        # 2. Panel de Filtros Avanzados Dinámicos
        with st.expander("🛠️ Mostrar Filtros Avanzados"):
            col_f1, col_f2, col_f3, col_f4 = st.columns(4)
            
            with col_f1:
                marcas_sel = st.multiselect("🏷️ Marca", sorted(df_mostrar['Marca'].dropna().unique()))
            
            with col_f2:
                importadores_sel = st.multiselect("🏢 Importadora", sorted(df_mostrar['Importador'].dropna().unique()))
                
            with col_f3:
                # Extraemos solo las químicas que no sean NaN o vacías
        ...
        # 3. Aplicación de los filtros al DataFrame
        if marcas_sel:
            df_mostrar = df_mostrar[df_mostrar['Marca'].isin(marcas_sel)]
        if importadores_sel:
            df_mostrar = df_mostrar[df_mostrar['Importador'].isin(importadores_sel)]
        if quimicas_sel:
            df_mostrar = df_mostrar[df_mostrar['Quimica_Bateria'].isin(quimicas_sel)]
        if rango_kwh:
            # Solo filtra si el usuario realmente movió la barra del slider
            if rango_kwh[0] > 0.0 or rango_kwh[1] < max_kwh:
                df_mostrar = df_mostrar[
                    (df_mostrar['Capacidad kWh'].notna()) & 
                    (df_mostrar['Capacidad kWh'] >= rango_kwh[0]) & 
                    (df_mostrar['Capacidad kWh'] <= rango_kwh[1])
                ]

        # 4. Mostrar resultados divididos por tecnología
...
# --- PESTAÑA 5: ECOSISTEMA DE BATERÍAS ---
with tab_ecosistema:
    ...
    if not df_top.empty:
        ...
        with col_f1:
            tipo_eco = st.radio("Filtra la tecnología para el ecosistema:", ["Todos", "BEV", "PHEV", "HEV"], horizontal=True)
        with col_f2:
        ...
        with col_m1:
            ...
            if not df_eco_filtrado.empty:
                ...
                st.info(f"No hay registros de ventas para el filtro seleccionado en {periodo_eco}.")

        with col_m2:
            st.subheader("Crecimiento de Energía (MWh por Año)")
            st.markdown("*(Este gráfico se mantiene histórico para comparar el crecimiento año tras año)*")
        ...
        st.divider()
        st.subheader(f"Cadena de Suministro ({periodo_eco})")
        ...
        with col_b1:
            if not df_eco_filtrado.empty:
                ...
                if not quimica_ventas.empty:
                    ...
                    st.info("No hay datos de química de batería para este periodo.")
        ...
        with col_b2:
            if not df_eco_filtrado.empty:
                ...
                if not fab_ventas.empty:
                    ...
                    st.info("No hay datos de fabricantes para este periodo.")

    else:
        st.warning("No se encontró el archivo 'Copia de modelos mas vendidos 2022 a 2026.xlsx'.")
